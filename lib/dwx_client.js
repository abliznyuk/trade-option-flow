...
class dwx_client {
  ...
  constructor({
    ...
    this.path_ticks = path.join(metatrader_dir_path, 'DWX', 'DWX_Ticks.txt');
    ...
    this.path_symbols = path.join(metatrader_dir_path, 'DWX', 'DWX_Symbols.txt');
    ...
    this.market_data = {};
    this.market_depth = {};
    this.ticks = {};               // per-symbol live buffer (last lookback window)
    this._tick_cursor = {};        // per-symbol cursor: highest time_msc emitted to handler
    this.bar_data = {};
    this.historic_data = {};
    ...
    this._pending_get_symbols = []; // promise resolvers awaiting GET_SYMBOLS response
    ...
    this.ticks_loop = this.check_ticks();
  ...
  /** Regularly checks the file for tick stream and triggers the event_handler.on_ticks() function.
   *
   * File format produced by DWX_Service_IS.mq5 CheckTicks() — positional schema (compact):
   *   { "<symbol>": [ [time_msc, bid, ask, last, volume_real, flags], ... ], ... }
   * MQL writes the last `lookback_sec` of ticks per symbol on every flush. We deduplicate
   * across reads using a per-symbol cursor (`_tick_cursor[symbol]` = highest emitted time_msc).
   *
   * The handler receives ticks already decoded into objects:
   *   { t: time_msc, b: bid, a: ask, p: last, v: volume_real, f: flags,
   *     is_trade: bool, is_buy: bool, is_sell: bool }
   */
  async check_ticks() {
    const delayMs = Math.max(1, Math.floor(this.sleep_delay * 1000));

    // MQL5 tick flags (see ENUM_TICK_FLAG)
    const TICK_FLAG_LAST = 8;
    const TICK_FLAG_BUY  = 32;
    const TICK_FLAG_SELL = 64;

    while (this.ACTIVE) {
      await sleep(delayMs);
      if (!this.START) continue;

      const text = await this.try_read_file(this.path_ticks);
      if (text.trim().length === 0 || text === this._last_ticks_str) continue;

      this._last_ticks_str = text;
      let data; try { data = JSON.parse(text); } catch { continue; }
      if (!data || typeof data !== 'object') continue;
      ...
      for (const symbol of Object.keys(data)) {
        ...
        const cursor = this._tick_cursor[symbol] ?? -1;

        // build full buffer (decoded), and detect which entries are "new" (past the cursor).
        const decoded = new Array(raw.length);
        const fresh = [];
        let maxT = cursor;
        for (let i = 0; i < raw.length; i++) {
          ...
          const obj = {
            ...
            v: Number(r[4]),
            f,
            is_trade: (f & TICK_FLAG_LAST) !== 0,
            is_buy:   (f & TICK_FLAG_BUY)  !== 0,
            is_sell:  (f & TICK_FLAG_SELL) !== 0,
          };
          decoded[i] = obj;
          if (t > cursor) {
            fresh.push(obj);
        ...
        // full snapshot kept in memory (so app code can read at any time via this.ticks[symbol])
        this.ticks[symbol] = decoded;

        if (fresh.length > 0) {
          this._tick_cursor[symbol] = maxT;
          if (this.event_handler && typeof this.event_handler.on_ticks === 'function') {
            try { this.event_handler.on_ticks(symbol, fresh); } catch {}
          }
        }
      }
    }
  ...
  async check_historic_data() {
    ...
    while (this.ACTIVE) {
      ...
      // symbols list (response to GET_SYMBOLS)
      text = await this.try_read_file(this.path_symbols);
      if (text.trim().length > 0) {
        let data; try { data = JSON.parse(text); } catch { data = null; }
        if (data && Array.isArray(data.symbols)) {
          // resolve any pending get_symbols() promises
          const pending = this._pending_get_symbols || [];
          this._pending_get_symbols = [];
          for (const resolve of pending) {
            try { resolve(data); } catch {}
          }
          if (this.event_handler && typeof this.event_handler.on_symbols_list === 'function') {
            try { this.event_handler.on_symbols_list(data); } catch {}
          }
        }
        await this.try_remove_file(this.path_symbols);
      }
    }
  }
  ...
  /** API: get_symbols — enumerate broker's symbol catalogue, optionally filtered by substring.
   *
   * Returns a Promise that resolves to { filter, symbols: [{name, digits, point, tick_size, contract_size}, ...] }.
   * Mirrors the GET_SYMBOLS one-shot command on the MQL side (DWX_Service_IS.mq5 GetSymbols).
   * Also fires event_handler.on_symbols_list(payload) when the response arrives.
   *
   * @param {string} [filter=''] substring to match symbol names against (case-sensitive). Empty -> all symbols.
   * @param {number} [timeoutMs=10000]
   */
  async get_symbols(filter = '', timeoutMs = 10000) {
    return new Promise((resolve, reject) => {
      let settled = false;
      const timer = setTimeout(() => {
        if (settled) return;
        settled = true;
        // remove our resolver from the pending list if it's still there
        const idx = this._pending_get_symbols.indexOf(wrappedResolve);
        if (idx >= 0) this._pending_get_symbols.splice(idx, 1);
        reject(new Error(`GET_SYMBOLS timed out after ${timeoutMs}ms`));
      }, timeoutMs);
      ...
      this._pending_get_symbols.push(wrappedResolve);
      this.send_command('GET_SYMBOLS', String(filter || ''));
    });
  }

  /** API: subscribe_ticks — live time & sales stream.
   *
   * Accepts either an array of symbol strings (uses MQL-side default lookback)
   * or an array of {symbol, lookback_sec} objects. Empty list unsubscribes from all.
   * Mirrors SUBSCRIBE_TICKS on the MQL side (DWX_Service_IS.mq5 SubscribeTicks),
   * which periodically dumps the last lookback window via CopyTicksRange.
   */
  async subscribe_ticks(specs) {
    const list = (specs || []).map(s => {
      if (typeof s === 'string') return s;
      if (s && s.symbol && s.lookback_sec) return `${s.symbol}:${parseInt(s.lookback_sec, 10)}`;
      if (s && s.symbol) return s.symbol;
      return '';
    }).filter(Boolean);
    // reset cursors on every (re)subscribe so the new bootstrap snapshot is delivered to handler in full
    this._tick_cursor = {};
    this.ticks = {};
    this.send_command('SUBSCRIBE_TICKS', list.join(','));
  }
